...
array<int, 3> find_predecessor(const array<int, 4>& edge, const vector<array<int, 3>>& Lvi) {
    ...
    // Find upper bound through exponential search
    int j = 1;
    while (j < r && Lvi[j][1] <= ti) {
        j *= 2;
    }
    
    int low = j / 2;
    int high = min(r - 1, j);
    int ans = low; // Start with the largest known good position
...
vector<pair<int,int>> short_fastest_path(vector<array<int, 4>>& graph,int S, int V, int E) {
    ...
    for(int i = 0; i < E; i++) {
        ...
        if (y!= array<int,3>{-1, -1, -1}) {
            ...
            if (sz == 0 || targetList.back() != T) {
                targetList.push_back(T);
            }

            if (targetList.size() >= 2) {
                array<int, 3>& last = targetList.back();              // T
                array<int, 3>& secondLast = targetList[targetList.size() - 2];  // Tr

                array<int, 3> dom = dominates(secondLast, last);
                if (dom == last) {
                    // T dominates Tr
                    targetList.erase(targetList.end() - 2);  // delete Tr (secondLast)
                } else if (dom == secondLast) {
                    // Tr dominates T
                    targetList.pop_back();  // delete T
                }
            }
...
int main(int argc, char* argv[]) {
 ...
 // Change this to your filename if needed
